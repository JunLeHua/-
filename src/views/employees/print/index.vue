...
<script>
import { getEmployeesInfo, getEmployeesJobInfo, getEmployeesBaseInfo } from '@/api/employees'
export default {
  data () {
    return {
      formData: {},
  ...
  methods: {
    ...
    async getJobDetail () {
      const userInfo = await getEmployeesBaseInfo(this.userId)
      const jobInfo = await getEmployeesJobInfo(this.userId) // 获取个人基本信息
